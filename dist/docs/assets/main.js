...
(() => {
    ...
    var Fe = (t, e, n, r) => { if (e && typeof e == "object" || typeof e == "function")
        for (let i of Oe(e))
            !Re.call(t, i) && i !== n && ne(t, i, { get: () => e[i], enumerable: !(r = Ie(e, i)) || r.enumerable }); return t; };
    ...
    var ae = Me((se, oe) => {
        (function () {
            ...
            t.tokenizer = function (e, n) { if (e == null || e == null)
                ...
                return e.map(function (v) { return new t.Token(t.utils.asString(v).toLowerCase(), t.utils.clone(n)); }); for (var r = e.toString().toLowerCase(), i = r.length, s = [], o = 0, a = 0; o <= i; o++) {
...
`, e);
            ...
            } return h = z, x = f, h.test(c) && x.test(c) && (h = w, c = c.replace(h, "")), T == "y" && (c = T.toLowerCase() + c.substr(1)), c; }; return function (R) { return R.update(V); }; }(), t.Pipeline.registerFunction(t.stemmer, "stemmer");
                ...
                for (var f = 0; f < d.length; f++) {
                    ...
                    for (var S = 0; S < L.length; S++)
                        for (var w = L[S], k = this.invertedIndex[w], _ = k._index, p = 0; p < u.fields.length; p++) {
                            var m = u.fields[p], H = k[m], A = Object.keys(H), j = w + "/" + m, $ = new t.Set(A);
                            ...
                            if (i[m].upsert(_, u.boost, function (Qe, Ce) { return Qe + Ce; }), !s[j]) {
                                for (var N = 0; N < A.length; N++) {
                                    var q = A[N], P = new t.FieldRef(q, m), z = H[q], W;
            ...
            } var c = Object.keys(r), y = [], I = Object.create(null); if (n.isNegated()) {
                c = Object.keys(this.fieldVectors);
                for (var l = 0; l < c.length; l++) {
                    var P = c[l], T = t.FieldRef.fromString(P);
                    r[P] = new t.MatchData;
                }
            } for (var l = 0; l < c.length; l++) {
            ...
            t.Builder = function () { this._ref = "id", this._fields = Object.create(null), this._documents = Object.create(null), this.invertedIndex = Object.create(null), this.fieldTermFrequencies = {}, this.fieldLengths = {}, this.tokenizer = t.tokenizer, this.pipeline = new t.Pipeline, this.searchPipeline = new t.Pipeline, this.documentCount = 0, this._b = .75, this._k1 = 1.2, this.termIndex = 0, this.metadataWhitelist = []; }, t.Builder.prototype.ref = function (e) { this._ref = e; }, t.Builder.prototype.field = function (e, n) { if (/\//.test(e))
                ...
                var o = i[s], a = this._fields[o].extractor, l = a ? a(e) : e[o], u = this.tokenizer(l, { fields: [o] }), d = this.pipeline.run(u), v = new t.FieldRef(r, o), f = Object.create(null);
    ...
    var U = class {
        constructor() { this.alwaysVisibleMember = null; this.createComponents(document.body), this.ensureActivePageVisible(), this.ensureFocusedElementVisible(), this.listenForCodeCopies(), window.addEventListener("hashchange", () => this.ensureFocusedElementVisible()); }
        ...
        ensureActivePageVisible() { let e = document.querySelector(".tsd-navigation .current"), n = e?.parentElement; for (; n && !n.classList.contains(".tsd-navigation");)
            n instanceof HTMLDetailsElement && (n.open = !0), n = n.parentElement; if (e) {
    ...
    var Be = { "&": "&amp;", "<": "&lt;", ">": "&gt;", "'": "&#039;", '"': "&quot;" };
    function K(t) { return t.replace(/[&<>"'"]/g, e => Be[e]); }
    var C = class {
        constructor(e) { this.el = e.el, this.app = e.app; }
    };
    var F = "mousedown", pe = "mousemove", B = "mouseup", J = { x: 0, y: 0 }, fe = !1, ee = !1, He = !1, D = !1, me = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
    ...
    document.addEventListener(pe, t => { if (ee && !D) {
        let e = F == "touchstart" ? t.targetTouches[0] : t, n = J.x - (e.pageX || 0), r = J.y - (e.pageY || 0);
        D = Math.sqrt(n * n + r * r) > 10;
    } });
    document.addEventListener(B, () => { ee = !1; });
    document.addEventListener("click", t => { fe && (t.preventDefault(), t.stopImmediatePropagation(), fe = !1); });
    var X = class extends C {
        ...
        onPointerUp(n) { D || (this.setActive(!0), n.preventDefault()); }
        onDocumentPointerDown(n) { if (this.active) {
            if (n.target.closest(".col-sidebar, .tsd-filter-group"))
                return;
            this.setActive(!1);
        } }
        onDocumentPointerUp(n) { if (!D && this.active && n.target.closest(".col-sidebar")) {
    ...
    var Y = class extends C {
        constructor(n) {
            super(n);
            this.key = `filter-${this.el.name}`, this.value = this.el.checked, this.el.addEventListener("change", () => { this.setLocalStorage(this.el.checked); }), this.setLocalStorage(this.fromLocalStorage()), ve.innerHTML += `html:not(.${this.key}) .tsd-is-${this.el.name} { display: none; }
`;
        }
        ...
        handleValueChange() { this.el.checked = this.value, document.documentElement.classList.toggle(this.key, this.value), this.app.filterChanged(), document.querySelectorAll(".tsd-index-section").forEach(n => { n.style.display = "block"; let r = Array.from(n.querySelectorAll(".tsd-index-link")).every(i => i.offsetParent == null); n.style.display = r ? "none" : "block"; }); }
    };
    var Z = class extends C {
        constructor(n) { super(n); this.summary = this.el.querySelector(".tsd-accordion-summary"), this.icon = this.summary.querySelector("svg"), this.key = `tsd-accordion-${this.summary.dataset.key ?? this.summary.textContent.trim().replace(/\s+/g, "-").toLowerCase()}`; let r = Q.getItem(this.key); this.el.open = r ? r === "true" : this.el.open, this.el.addEventListener("toggle", () => this.update()); let i = this.summary.querySelector("a"); i && i.addEventListener("click", () => { location.assign(i.href); }), this.update(); }
        update() { this.icon.style.transform = `rotate(${this.el.open ? 0 : -90}deg)`, Q.setItem(this.key, this.el.open.toString()); }
    };
    function ge(t) { let e = Q.getItem("tsd-theme") || "os"; t.value = e, ye(e), t.addEventListener("change", () => { Q.setItem("tsd-theme", t.value), ye(t.value); }); }
    function ye(t) { document.documentElement.dataset.theme = t; }
    var Le;
    ...
    async function xe() { let t = document.getElementById("tsd-nav-container"); if (!t || !window.navigationData)
        ...
        we(s, t, []); window.app.createComponents(t), window.app.ensureActivePageVisible(); }
